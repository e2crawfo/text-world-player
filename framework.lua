...
function random_quest()
    ...
    -- Pick some misleading text to add to the quest description.
...
-- Here we're adding words to the symbol_mapping from the quests
...
-- Here we're adding words to the symbol_mapping from the
-- ``extra_vocab`` table
...
-- read in text data from - nicely tokenized
...
function convert_text_to_bow(input_text)
    ...
    for j, line in pairs(input_text) do
        ...
        print("Tokenized Line: " .. table.concat(tokens, ' '))
    end
