...
local function make_vector_networks (
        tree, matrix_networks, word_vecs, W, nl_class, predictor, train_rep)

    local vector_networks = {}
    local prediction_networks = {}

    local function f(tree, top_level)
        if tree.terminal then
        ...
        if train_rep or top_level then
            -- Add a prediction network if we're in rep-training mode, or if we're at a
            -- top-level constituent (i.e. sentence).
...
do
    ...
    function MVRNN:__init(
             n, r, words, nl_class, predictor, rep_predictor, rep_criterion,
             combine_mode, rv_func, wv_func, allow_new_words)
        ...
        self.rep_predictor = rep_predictor
        self.rep_criterion = rep_criterion

        self.train_rep = self.rep_predictor and self.rep_criterion
    ...
    function MVRNN:updateOutput(text)
        ...
        for i=1,size do
            ...
            if not forward_net then
                ...
                if not train_rep then
                    self.forward_networks[txt_i] = forward_net
                end
            end

            result = forward_net:forward(torch.Tensor()):clone()
            result = result:reshape(1, result:size(1))

            if not self.output then
                self.output = result
            else
                self.output = torch.cat(self.output, result, 1)
            end
        end
    ...
    function MVRNN:accGradParameters(text, gradOutput, scale)
        ...
        if self.train_rep then
            -- Only train the predictor, we'll train the representation separately
            for i=1,size do
                pts, txt_i = self:parse(text[i])
                forward_net = self.forward_networks[txt_i]

                -- Assumes forward_net:forward has already been called.
                local rep = forward_net:get(#forward_net-1).output
                forward_net:get(#forward_net):backward(rep, gradOutput[i])
            end
        else
    ...
    -- Basically, this should be called in addition to calling backward.
    -- Accumulates gradients for the representation.
    function MVRNN:accGradReprParameters(text, target)
        local batch_mode, size, input, pts_i, trgt, n_sentences, constituent_subtrees, nets_to_train

        if not self.train_rep then
            error("Cannot call ``accGradReprParameters`` when not in representation training mode." ..
                  "Need to supply a representation predictor and criterion to constructor.")
        end
        ...
        for i=1,size do
            ...
            n_sentences = table_length(pts_i)
            ...
            for s, tree in pairs(pts_i) do
                ...
                for t, _ in pairs(constituent_subtrees) do
                    ...
                    nets_to_train[#nets_to_train+1] = {
                        self.prediction_networks[t], trgt, 1.0/(n_sentences * size), tostring(t)}
                end
            end
        end
        ...
        for i, net in ipairs(nets_to_train) do
            ...
            err = self.rep_criterion:forward(prediction, net[2])
            net[1]:backward(
                torch.Tensor(),
                self.rep_criterion:backward(prediction, net[2]),
                net[3])
        end
    end
    ...
    function MVRNN:add_word(word)
        ...
        if not self.word_vecs[word] then
            local u_shape, v_shape, w_shape = self.u_shape, self.v_shape, self.w_shape
    ...
    function MVRNN:parse(text)
        ...
        if not self.parse_trees[text] then
            ...
            for s, tree in pairs(parse_trees) do
                ...
                predictor = self.train_rep and self.rep_predictor or self.predictor

                vector_networks, prediction_networks = make_vector_networks(
                    tree, matrix_networks, self.word_vecs, self.W,
                    self.nl_class, predictor, self.train_rep)
    ...
    function MVRNN:parameters()
        ...
        local pred_params, pred_grad_params = self.rep_predictor:parameters()
        ...
        if self.rep_predictor ~= self.predictor then
...
local function test_mvrnn_simple()
    print("Testing MV-RNN...")
    n = 10
    r = 3
    nl_class = nn.Tanh
    random_vec_func = function (size) return torch.Tensor(size):normal(0, 1) end
    combine_mode = MEAN

    predictor = nn.Linear(n, 1)
    rep_predictor = nn.Linear(n, 1)
    rep_criterion = nn.MSECriterion()
    ...
    parse_trees = parse.parse_sentences({all_text})
    ...
    -- Done getting all words.

    local mvrnn = MVRNN(
        n, r, all_words, nl_class, predictor, rep_predictor, rep_criterion,
        combine_mode, random_vec_func, random_vec_func, false)
...
mvrnn = {
    MVRNN=MVRNN,
    make_mvrnn = function (
            n, r, words, nl_class, predictor, rep_predictor,
            rep_criterion, combine_mode, rv_func, wv_func, allow_new_words)

        return MVRNN(
            n, r, words, nl_class, predictor, rep_predictor,
            rep_criterion, combine_mode, rv_func, wv_func, allow_new_words), n
    end,
